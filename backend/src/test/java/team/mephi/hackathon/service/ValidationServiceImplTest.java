...
class ValidationServiceImplTest {
  ...
  void validateInn_withValid10DigitInn_shouldPass() {
    assertDoesNotThrow(() -> validationService.validateTransaction(createDto("1234567890", null)));
  }

  @Test
  void validateInn_withValid12DigitInn_shouldPass() {
    assertDoesNotThrow(
        () -> validationService.validateTransaction(createDto("123456789012", null)));
  }
  ...
  void validatePhone_withValid8Format_shouldPass() {
    assertDoesNotThrow(() -> validationService.validateTransaction(createDto(null, "81234567890")));
  }
  ...
  void validatePhone_withInvalidFormat_shouldThrowException() {
    ValidationException exception =
        assertThrows(
            ...
            () -> validationService.validateTransaction(createDto(null, "71234567890")));
    assertEquals(
        "Телефон должен быть в формате +7XXXXXXXXXX или 8XXXXXXXXXX", exception.getMessage());
  }
