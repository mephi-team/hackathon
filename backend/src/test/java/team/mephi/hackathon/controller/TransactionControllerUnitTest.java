...
class TransactionControllerUnitTest {
  ...
  void createTransaction_shouldReturnCreatedDto() {
    ...
    resp.setId(id);
    when(transactionService.createTransaction(any(TransactionRequestDto.class))).thenReturn(resp);
  ...
  void searchTransactions_shouldReturnList() {
    ...
    webTestClient
        .get()
        .uri(uriBuilder -> uriBuilder.path("/api/transactions").queryParam("status", "NEW").build())
        .exchange()
        .expectStatus()
        .isOk()
